...
void Game::Draw()
{
    ...
    if (buttonRestart.IsPressed())
    {
        ...
        mesh_smile->Add(q_smilePressed, libVec3(screenSize.x / 2.0f, pOffset.y + TILE_SIZE + halfTile / 2.0f, 0.0f));
    }
    else
    {
        libQuad q_smile(libVertex(-TILE_SIZE, -TILE_SIZE, 0.0f, 0.0f), libVertex(TILE_SIZE, TILE_SIZE, 1.0f, 1.0f));
        mesh_smile->Add(q_smile, libVec3(screenSize.x / 2.0f, pOffset.y + TILE_SIZE + halfTile / 2.0f, 0.0f));
    }
