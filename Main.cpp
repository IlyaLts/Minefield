...
void Restart()
{
    ...
    for (int i = 0; i < fieldSize.x; i++)
    {
        for (int j = 0; j < fieldSize.y; j++)
        {
            if (field[i][j].type == Tile::MINE) continue;
...
bool Init()
{
    LIB_CHECK(engine->GetTexture(t_smile, DATA_PACK "Textures/Smile.png"));
    LIB_CHECK(engine->GetTexture(t_smileClick, DATA_PACK "Textures/SmileClick.png"));
    LIB_CHECK(engine->GetTexture(t_smileWin, DATA_PACK "Textures/SmileWin.png"));
    LIB_CHECK(engine->GetTexture(t_smileLost, DATA_PACK "Textures/SmileLost.png"));
    LIB_CHECK(engine->GetTexture(t_tile, DATA_PACK "Textures/Tile.png"));
    LIB_CHECK(engine->GetTexture(t_tileOpen, DATA_PACK "Textures/TileOpen.png"));
    LIB_CHECK(engine->GetTexture(t_mine, DATA_PACK "Textures/Mine.png"));
    LIB_CHECK(engine->GetTexture(t_flag, DATA_PACK "Textures/Flag.png"));
    LIB_CHECK(engine->GetTexture(t_question, DATA_PACK "Textures/Question.png"));
    LIB_CHECK(engine->GetSprite(t_boom, DATA_PACK "Textures/Boom/Boom.png"));
...
bool Draw()
{
    ...
    for (int i = 0; i < fieldSize.x; i++)
    {
        for (int j = 0; j < fieldSize.y; j++)
        {
            ...
            // Mines
            else if (gameState == LOST && ((field[i][j].type == Tile::MINE && field[i][j].state != Tile::FLAGGED) || (field[i][j].type != Tile::MINE && field[i][j].state == Tile::FLAGGED)))
            {
                ...
                {
                    engine->Draw2DLine(libVertex(x, y, LIB_COLOR_RED), libVertex(x + TILE_SIZE, y + TILE_SIZE, LIB_COLOR_RED), 3.0f);
                    engine->Draw2DLine(libVertex(x, y + TILE_SIZE, LIB_COLOR_RED), libVertex(x + TILE_SIZE, y, LIB_COLOR_RED), 3.0f);
                }
            }
            // Flags
            if ((gameState != LOST && field[i][j].state == Tile::FLAGGED) || (gameState == LOST && field[i][j].type == Tile::MINE && field[i][j].state == Tile::FLAGGED))
            {
    ...
    // Mine explosion
    if (gameState == LOST && t_boom->IsPlaying()) t_boom->Draw2DQuad(q_boom, boom.x, boom.y);
...
bool Frame()
{
    if (engine->IsKeyPressed(LIBK_ESCAPE)) engine->Stop();
    if (engine->IsKeyPressed(LIBK_F1)) libDialog::Information(libFormat("Minefield version: %s", MINEFIELD_VERSION), help);
    if (engine->IsKeyPressed(LIBK_F12)) engine->TakeScreenshot();

    // Restart button
    buttonRestart.Update();

    if (buttonRestart.IsPressed()) buttonRestart.SetTexture(t_tileOpen);
    if (buttonRestart.IsReleased()) Restart();

    // Sets smile button to default state
    if (!engine->IsKey(LIBK_MOUSE_LEFT))
    {
        if (gameState == IN_GAME) t_curSmile = t_smile;
    ...
    for (int i = 0, clicked = false; i < fieldSize.x; i++)
    {
        for (int j = 0; j < fieldSize.y; j++)
        {
            ...
            if (clicked || gameState != IN_GAME) continue;

            if (field[i][j].button.IsPressed() && (field[i][j].state == Tile::CLOSED || field[i][j].state == Tile::QUESTIONED))
            ...
            // Opens a tile
            if (field[i][j].button.IsHovered() && engine->IsKeyReleased(LIBK_MOUSE_LEFT) && (field[i][j].state == Tile::CLOSED || field[i][j].state == Tile::QUESTIONED))
            {
                ...
                if (field[i][j].type == Tile::MINE)
                {
                    ...
                    for (int i = 0; i < fieldSize.x; i++)
                    {
                        for (int j = 0; j < fieldSize.y; j++)
                        {
                            if ((field[i][j].type == Tile::MINE && field[i][j].state != Tile::FLAGGED) || (field[i][j].type != Tile::MINE && field[i][j].state == Tile::FLAGGED))
                            {
                                field[i][j].button.SetTexture(t_tileOpen);
                            }
                        }
                            ...
                            for (int j = 0; j < fieldSize.y; j++)
                            {
                                ...
                                for (int x = -1; x <= 1; x++)
                                {
                                    for (int y = -1; y <= 1; y++)
                                    {
                                        ...
                                        if (i + x < 0 || i + x >= fieldSize.x) continue;
                                        if (j + y < 0 || j + y >= fieldSize.y) continue;

                                        if (field[i + x][j + y].state == Tile::OPEN && !field[i + x][j + y].nearestMines)
                                        {
                                            if (field[i][j].state == Tile::FLAGGED) shownMinesLeft++;
                ...
                if (isGameFinished)
                {
                    ...
                    for (int i = 0; i < fieldSize.x; i++)
                    {
                        for (int j = 0; j < fieldSize.y; j++)
                        {
                            if (field[i][j].type == Tile::MINE && field[i][j].state != Tile::OPEN && field[i][j].state != Tile::FLAGGED)
                            {
                                field[i][j].state = Tile::FLAGGED;
                                shownMinesLeft--;
                            }
